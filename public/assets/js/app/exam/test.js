...
function not_answer()
{
  ...
  check_status_doubt(rel);
  save();
}

function check_status_doubt(rel)
{
  var status_doubt = $('#doubt_'+rel).val();
  if(status_doubt == 'N'){
    $('#btn-doubtfull').text('Ragu-Ragu');
  }else{
    $('#btn-doubtfull').text('Tidak Ragu');
  }
}
...
function next()
{
  ...
  $('#btn-doubtfull').attr('rel', next);

  check_status_doubt(next);
  check_last(next);
...
function save()
{
  ...
  for(var i=1; i<=total_question; i++){
    var index1 = 'option_'+i;
    var index2 = 'doubt_'+i;
    var answer = form[index1];
    var doubt  = form[index2];

    if(answer != undefined){
      if(doubt == 'Y'){
        answer_result += '<button type="button" id="btn_'+i+'" class="btn btn-warning btn-sm mr-2 mt-1" onclick="return page('+i+')">'+i+'. '+answer+'</button>';
      }else{
        answer_result += '<button type="button" id="btn_'+i+'" class="btn btn-success btn-sm mr-2 mt-1" onclick="return page('+i+')">'+i+'. '+answer+'</button>';
      }
    }else{
      answer_result += '<button type="button" id="btn_'+i+'" class="btn btn-light btn-sm mr-2 mt-1" onclick="return page('+i+')">'+i+'. -</button>';
    }
  }
...
function finish()
{
  $.ajax({
    ...
    success: function(data){
      if(data.status){
        location.href = base_url+"exam/pg";
      }else{
        message('warning','Perhatian!','Anda masih memiliki jawaban yang masih ragu-ragu.');
      }
    }
  });
}
